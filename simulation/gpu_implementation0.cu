...
void GPU_Implementation0::transfer_ponts_to_device(const std::vector<icy::Point> &points)
{
    cudaError_t err;
    err = cudaMemcpy(gpu_points_, (void*)points.data(), sizeof(icy::Point)*points.size(), cudaMemcpyHostToDevice);
    if(err != cudaSuccess)
    {
...
void GPU_Implementation0::cuda_transfer_from_device(std::vector<icy::Point> &points)
{
    cudaError_t err;
    err = cudaMemcpy((void*)points.data(), gpu_points_, sizeof(icy::Point)*points.size(), cudaMemcpyDeviceToHost);
    if(err != cudaSuccess)
    {
