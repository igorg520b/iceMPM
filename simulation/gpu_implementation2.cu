...
__device__ Eigen::Vector2f *gpu_grid_momentum, *gpu_grid_velocity;
...
void GPU_Implementation2::cuda_update_nodes(const int nGridNodes,float indenter_x, float indenter_y)
{
    cudaError_t err;
...
__global__ void v2_kernel_p2g(const int nPoints)
{
    ...
    const float &vol = gprms.ParticleVolume;
    const float &h = gprms.cellsize;
    const float &h_inv = gprms.cellsize_inv;
    const float &Dinv = gprms.Dp_inv;
...
//    p.NACC_alpha_p = gpu_pts_NACC_alpha_p[pt_idx];
    ...
    Eigen::Matrix2f stress = - (dt * vol) * (Dinv * dFe);
    Eigen::Matrix2f affine = stress + particle_mass * p.Bp;
    ...
    Eigen::Vector2f base_coord(i0,j0);
    Eigen::Vector2f fx = p.pos*h_inv - base_coord;

    Eigen::Vector2f v0(1.5-fx[0],1.5-fx[1]);
    Eigen::Vector2f v1(fx[0]-1.,fx[1]-1.);
    Eigen::Vector2f v2(fx[0]-.5,fx[1]-.5);
    ...
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 3; j++)
        {
            float Wip = w[i][0]*w[j][1];
            Eigen::Vector2f dpos((i-fx[0])*h, (j-fx[1])*h);
            Eigen::Vector2f incV = Wip*(p.velocity*particle_mass+affine*dpos);
...
__global__ void v2_kernel_g2p(const int nPoints)
{
    ...
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 3; j++)
        {
            Eigen::Vector2f dpos = Eigen::Vector2f(i, j) - fx;
            ...
            const Eigen::Vector2f &grid_v = node.velocity;
            p.velocity += weight * grid_v;
            p.Bp += (4.f*h_inv)*weight *(grid_v*dpos.transpose());
        }

    // Advection
    p.pos += dt * p.velocity;
//    p.Fe = (Eigen::Matrix2f::Identity() + dt*p.Bp) * p.Fe;
...
{
    const Eigen::Matrix2f &FModifier = p.Bp;
